...
const DEFAULT_COLOR = '#ffffff'; // White for Second Brain nodes
    ...
    return (
        <div className="flex flex-col lg:flex-row min-h-[calc(85dvh)] lg:h-[calc(100dvh-3rem)] w-full">
            ...
            <div
                ...
                className="flex-1 lg:w-full min-h-[50dvh] relative bg-secondary border border-white/5 shadow-sm overflow-hidden"
            >
                ...
                {/* Control Overlay */}
                <div className="absolute top-6 left-12 z-50 bg-black backdrop-blur-md p-3 rounded-xl border border-white/5 shadow-sm flex flex-col gap-2 pointer-events-auto">
                    <button
                        onClick={handleCreateNew}
                        className="flex items-center gap-2 px-3 py-1.5 bg-white/5 hover:bg-white/5 text-white rounded-lg text-sm font-medium transition-colors w-full justify-center"
                    >
                    ...
                    <button
                        onClick={resetCamera}
                        className="flex items-center gap-2 px-3 py-1.5 bg-secondary hover:bg-white/5 text-white/60 rounded-lg text-sm font-medium border border-white/5 transition-colors w-full justify-center"
                    >
                        <Maximize className="w-4 h-4" /> Reset View
                    </button>

                    <div className="flex items-center justify-between gap-3 px-1 py-1 w-full bg-white/5 rounded-lg border border-white/5">
                        <span className="text-[11px] font-bold text-white/40 flex items-center gap-1.5 ml-1.5">
                            Labels
                        </span>
                        <button
                            onClick={() => setShowLabels(!showLabels)}
                            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${showLabels ? 'bg-army-600' : 'bg-white/5'
                                }`}
                        >
                            <span
                    ...
                    {!loading && (
                        <div className="mt-2 pt-2 border-t border-white/5 text-[10px] text-white/40 text-center">
            ...
            <div className="w-full lg:w-1/3 h-[50dvh] lg:h-full flex justify-start items-end bg-transparent flex flex-col overflow-hidden">
                {!selectedNodeId && !isEditing ? (
                    ...
                    <>
                        {/* Header Actions */}
                        <div className="w-full p-4 border-b border-white/5 flex justify-between items-center bg-white/5">
                            ...
                            <div className="flex gap-2">
                                {!isEditing && selectedNodeId && (
                                    <>
                                        <button onClick={handleEditCurrent} className="p-1.5 text-white/40 hover:text-white/40 bg-secondary border border-white/5 rounded-md transition-colors shadow-sm">
                                            <Edit3 className="w-4 h-4" />
                                        </button>
                                        <button onClick={handleDelete} className="p-1.5 text-white/40 hover:text-red-600 bg-secondary border border-white/5 rounded-md transition-colors shadow-sm">
                                ...
                                {isEditing && (
                                    <>
                                        <button
                                            ...
                                            className="px-3 py-1.5 text-sm text-white/60 hover:bg-white/5 rounded-md transition-colors"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            onClick={handleSave}
                                            disabled={isSaving}
                                            className="px-3 py-1.5 text-sm flex items-center gap-2 bg-army-500 hover:bg-army-600 text-white rounded-md transition-colors disabled:opacity-50"
                                        >
                            ...
                            {isEditing ? (
                                <div className="space-y-2 flex flex-col h-full">
                                    <input
                                        ...
                                        className="w-full text-lg font-bold px-1 py-1 text-white focus:outline-none focus:ring-2 focus:ring-army-500/50 transition-shadow"
                                        placeholder="e.g. Backpropagation Algorithm"
                                    />
                                    <div className="flex items-center gap-2 border-b border-white/5">
                                        <label className="text-xs font-semibold text-white/40 mb-1">Tags</label>
                                        <input
                                            type="text"
                                            value={editFormData.tags}
                                            onChange={e => setEditFormData({ ...editFormData, tags: e.target.value })}
                                            className="w-full px-1 py-1 text-xs text-white focus:outline-none focus:ring-2 focus:ring-army-500/50 transition-shadow"
                                    ...
                                    <div className="relative flex flex-col items-start z-40 pt-2">
                                        <button
                                            ...
                                            className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/5 text-white/80 rounded-lg text-sm font-semibold transition-colors border border-white/5"
                                        ...
                                        {isLinkDropdownOpen && (
                                            <div className="absolute top-full left-0 mt-2 w-72 bg-secondary border border-white/5 rounded-xl shadow-xl p-2 flex flex-col gap-2">
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        ...
                                                        autoFocus
                                                        className="w-full bg-white/5 border border-white/5 rounded-lg pl-9 pr-3 py-1.5 text-sm text-white outline-none focus:border-army-500 focus:ring-1 focus:ring-army-500 transition-shadow"
                                                    />
                                                </div>
                                                <div className="max-h-48 overflow-y-auto custom-scrollbar flex flex-col gap-1 pr-1">
                                                    {records
                                                        ...
                                                        .map(node => (
                                                            <div key={node.id} className="group flex items-center justify-between p-2 hover:bg-white/5 rounded-lg transition-colors">
                                                                <span className="text-sm text-white/80 truncate pr-2 flex-1" title={node.title}>{node.title}</span>
                                                                <button
                                                                    type="button"
                                                                    onClick={() => {
                                                                        handleInsertLink(node.id, node.title);
                                                                    }}
                                                                    className="opacity-0 group-hover:opacity-100 flex-shrink-0 p-1.5 bg-army-50 text-army-600 hover:bg-army-100 rounded-md transition-all border border-army-200"
                                                    ...
                                                    {records.filter(r => r.id !== selectedNodeId && r.title.toLowerCase().includes(linkSearchQuery.toLowerCase())).length === 0 && (
                                                        <p className="text-xs text-white/40 p-2 text-center">No nodes found.</p>
                                                    )}
                                                </div>
                                            </div>
                                        )}
                                    ...
                                    <div className="flex-1 flex flex-col">
                                        <label className="block text-xs font-semibold text-white/40 mb-1">Content</label>
                                        <textarea
                                            ref={textareaRef}
                                            value={editFormData.content}
                                            onChange={e => setEditFormData({ ...editFormData, content: e.target.value })}
                                            className="flex-1 w-full px-2 py-1 text-white focus:outline-none focus:ring-2 focus:ring-army-500/50 transition-shadow resize-none font-mono text-sm"
                                ...
                                selectedRecord && (
                                    <div className="space-y-4">
                                            ...
                                            {selectedRecord.tags && (
                                                <div className="flex flex-wrap gap-2 mt-2">
                                                    {selectedRecord.tags.split(',').map((tag, i) => (
                                                        <span key={i} className="px-2 py-0.5 bg-army-100 text-army-500 rounded text-xs font-medium">
                                                            #{tag.trim()}
                                                        </span>
                                                    ))}
                                                </div>
                                        ...
                                        <div className="prose prose-slate max-w-none prose-sm border-t border-white/5 pt-2">
                                            <ReactMarkdown
                                                ...
                                                components={{
                                                    a: ({ node, href, children, ...props }) => {
                                                        if (href && href.startsWith('#')) {
                                                            ...
                                                            return (
                                                                <a
                                                                    ...
                                                                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold bg-army-50 text-army-600 hover:bg-army-100 no-underline transition-colors border border-army-200 mx-1 align-middle"
                                                                    title="Jump to Node"
                                                                    {...props}
                                                                >
                                                                    {children}
                                                        ...
                                                        return (
                                                            <a
                                                                ...
                                                                className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-army-50/50 border border-army-100 rounded-md hover:border-army-500 transition-all no-underline text-army-900 align-middle mx-0.5 text-[11px] font-medium"
                                                                {...props}
                                                            >
                                                        ...
                                                        if (props.type === 'checkbox') {
                                                            return (
                                                                <input
                                                                    ...
                                                                    readOnly
                                                                    className="w-4 h-4 rounded border-white/5 text-army-600 focus:ring-army-500 cursor-pointer mr-2 align-middle"
                                                                    onClick={(e) => {
                                                                        e.stopPropagation();
                                                    ...
                                                    h1: ({ children }) => <h1 className="text-3xl font-extrabold text-white mb-6 mt-8 border-b pb-2 border-white/5">{children}</h1>,
                                                    h2: ({ children }) => <h2 className="text-2xl font-bold text-white/80 mb-4 mt-6">{children}</h2>,
                                                    h3: ({ children }) => <h3 className="text-xl font-bold text-white/80 mb-3 mt-5">{children}</h3>,
                                                    ...
                                                    p: ({ children }) => <p className="mb-4 leading-relaxed text-white/80 whitespace-pre-wrap">{children}</p>,
